...
생각
문자열을 받아서 가장 많이 나타난 문자가 뭔가?
소문자로만 나타나니까 배열을 하나 만들어서 거기서 카운터를 세고 가장 많은 알파뱃을 표시
++
소문자만 나오는데 거기서 97을 빼면 배열의 0번부터 25번까지 알파벳 26개에 접근가능
+++
배열 초기화는 fill사용해야함,
또한 공백을 입력받기 위해 getline사용
...
int main()
{
    ...
    for(int i = 0; i < T; i++) {
        ...
        int maxVal = *max_element(alpha, alpha + 26);
        int maxCount = 0;
        char maxChar = '?';
